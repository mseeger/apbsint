...
% ULTIMATE DEBUG!!
%u = zeros(size(B,2),1);
%for i = 1:size(deb_ept_pi,2)
%  pi = deb_ept_pi(:,i);
%  b = deb_ept_beta(:,i);
%  u = postmean(X,s2,B,t,pi,y,b,innerExact,ninMVM,u, ...
%	       opts.innerOutput);
%  deb_glm_h(:,i) = B*u-t;
%  args = {X,1/s2,B,pi};
%  [z,zu,ldA] = diaginv_full(args{:});
%  deb_glm_rho(:,i) = z;
%end
% END ULTIMATE DEBUG
...
b = feval(pot,zeros(q,1));
b = b(:,4).*tau; % CHANGE M.S.: Multipl. with TAU makes more sense
...
for i = 1:nout
  ...
  % DEBUG
  deb_glm_pi = [deb_glm_pi pi(:)];
  deb_glm_beta = [deb_glm_beta b(:)];

  % INSERT M.S. !!
  h_q = B*u-t;
  if i>1
    % DEBUG:
    deb_glm_h = [deb_glm_h h_q];
    deb_glm_rho = [deb_glm_rho z(:)];
    % Compute DELTA
    delta = max(ept.reldiff([h_old(:); sqrt(z_old(:))],[h_q(:); ...
		    sqrt(z(:))]));
    fprintf(1,'Iter. %d: delta=%f\n',i,delta)
  end
  h_old = h_q; z_old = z;
  % END INSERT M.S.
